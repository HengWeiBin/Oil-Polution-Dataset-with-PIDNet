...
class ElanRoad(BaseDataset):
    def __init__(self, 
        ...
        super(ElanRoad, self).__init__(ignore_label, base_size,
    ...
    def read_files(self):
        ...
        if 'test' in self.list_path:
            ...
            for item in self.img_list:
                ...
                if self.hsv_aug:
                    self.extendAugmentFile(new_files, 'HSV', 3)
                if self.mba_aug:
                    self.extendAugmentFile(new_files, 'MBA', 7)
    ...
    def __getitem__(self, index):
        ...
        if 'HSV' in item['aug']:
            ...
            image = self.random_HSV_augment(image, label, gamma, self.label_mapping[1])
        if 'MBA' in item['aug']:
            alpha = random.uniform(0, 0.3)
            image = self.mixingBackgroundAugmentation(image, label, alpha, self.label_mapping[1])
